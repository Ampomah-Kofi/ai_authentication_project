...
const assignment = read("deployment/public/api/study/assignment.php");
...
const schema = read("deployment/database/mariadb_schema.sql");
...
requireMatch("deployment/public/index.html", html, /Please do not close this window until you finish the survey\./);
requireMatch("deployment/public/index.html", html, /id="toSurveyBtn"[^>]*target="_blank"/);
requireMatch("deployment/public/index.html", html, /rel="noopener noreferrer"/);
rejectMatch("deployment/public/index.html", html, /window\.location\.href\s*=\s*url/);
requireMatch("deployment/public/index.html", html, /sessionStorage\.setItem\(HANDOFF_SESSION_KEY/);
requireMatch("deployment/public/index.html", html, /restoreCompletedHandoff\(\)/);
requireMatch("deployment/public/index.html", html, /data_saved:\s*dataSaved/);
rejectMatch("deployment/public/index.html", html, /sessionStorage\.setItem\([^\n]*JSON\.stringify\(data\)/);
requireMatch("deployment/public/index.html", html, /Local test mode:[\s\S]*?return false;/);
requireMatch("deployment/public/index.html", html, /if \(!data\.consented_at \|\| data\.finished_at\) return;/);
requireMatch("deployment/public/index.html", html, /schema_version:\s*"1\.1\.0"/);
requireMatch("deployment/public/index.html", html, /server_permuted_block/);
requireMatch("deployment/public/index.html", html, /sessionStorage\.setItem\(ASSIGNMENT_SESSION_KEY/);
requireMatch("deployment/public/study-config.js", config, /apiBaseUrl:\s*""/);
requireMatch("deployment/public/study-config.js", config, /assignmentPath:\s*"\/api\/study\/assignment\.php"/);
...
requireMatch("deployment/src/bootstrap.php", bootstrap, /verify_database_assignment/);
requireMatch("deployment/public/api/study/assignment.php", assignment, /secure_shuffle/);
requireMatch("deployment/public/api/study/assignment.php", assignment, /random_int\(0, \$index\)/);
requireMatch("deployment/public/api/study/assignment.php", assignment, /\$blockSize\s*=\s*4/);
requireMatch("deployment/public/api/study/assignment.php", assignment, /FOR UPDATE/);
requireMatch("deployment/public/api/study/assignment.php", assignment, /WHERE pid = \? LIMIT 1/);
requireMatch("deployment/public/api/study/assignment.php", assignment, /dirname\(__DIR__,\s*3\)\s*\.\s*'\/src\/bootstrap\.php'/);
...
requireMatch("deployment/public/api/study/task-events.php", events, /verify_database_assignment/);
requireMatch("deployment/public/api/study/task-withdrawals.php", withdrawals, /verify_database_assignment/);
requireMatch("deployment/database/mariadb_schema.sql", schema, /CREATE TABLE IF NOT EXISTS study_assignment_state/);
requireMatch("deployment/database/mariadb_schema.sql", schema, /CREATE TABLE IF NOT EXISTS study_assignment_slots/);
requireMatch("deployment/database/mariadb_schema.sql", schema, /UNIQUE KEY uq_assignment_pid \(pid\)/);
