...
4. Set each field to capture the URL parameter of the same name. The task sends participants to the survey with this format: `?pid=P-XXXXXXXXXX&condition=A_fresh&placement=top`.
...
6. Publish the survey, then paste the published survey link into `surveyBaseUrl` in `study-config.js`.
